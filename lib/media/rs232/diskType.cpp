...
mediatype_t MediaType::discover_mediatype(const char *filename)
{
    ...
    if (l > 4 && filename[l - 4] == '.')
    {
        ...
        // BIN/ROM/INT/ITV/COL are all "load this straight into cart memory
        // and reset" formats on every current BUILD_RS232 platform
        // (Intellivision .bin+.cfg pairs and Intellicart .rom images, Atari
        // 2600 .bin homebrew, CoCo cartridge images, ColecoVision .col) --
        // there is no disk-image convention on any of them that also uses
        // these extensions, so detecting MEDIATYPE_ROM by extension alone is
        // unambiguous. This replaces the previous
        // disksize==8192||16384||32768 heuristic (rs232Disk::mount()), which
        // misfired on any file of those exact sizes regardless of extension
