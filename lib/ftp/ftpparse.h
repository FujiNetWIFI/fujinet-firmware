/**
 * ftpparse - parse one line of FTP LIST output.
 *
 * D. J. Bernstein's ftpparse, vendored. It lived inline at the top of
 * fnFTP.cpp until it was split out here so that ftp_dir_parse.cpp -- and
 * the ftp_directory_tests that drive it -- can link it without pulling in
 * fnFTP.h -> Protocol.h -> bus.h. The code below is unchanged from
 * what was in fnFTP.cpp; see ftpparse.cpp for the upstream notice and the
 * list of listing formats it covers.
 */
