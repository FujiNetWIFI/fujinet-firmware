            ...
            if(s_pageNr == 1){ // identificaton header
              int idx = VORBIS_specialIndexOf(inbuf, "vorbis", 10);
                ...
                if(idx == 1){
                    ...
                    Debug_printf("no \"vorbis\" something went wrong %i\n", len);
                }
                s_pageNr = 3;
            }
...
int parseVorbisFirstPacket(uint8_t *inbuf, int16_t nBytes){ // 4.2.2. Identification header
    ...
    if(s_blocksizes[0] < 64){
        Debug_println("blocksize[0] too low");
        return -1;
    }
    if(s_blocksizes[1] < s_blocksizes[0]){
        Debug_println("s_blocksizes[1] is smaller than s_blocksizes[0]");
        return -1;
    }
    if(s_blocksizes[1] > 8192){
        Debug_println("s_blocksizes[1] is too big");
        return -1;
    }

    if(channels < 1 || channels > 2){
        Debug_printf("nr of channels is not valid ch=%i\n", channels);
        return -1;
    }
    s_vorbisChannels = channels;

    if(sampleRate < 4096 || sampleRate > 64000){
        Debug_printf("sampleRate is not valid sr=%i\n", sampleRate);
        return -1;
    }
...
int parseVorbisComment(uint8_t *inbuf, int16_t nBytes){      // reference https://xiph.org/vorbis/doc/v-comment.html
    ...
    if(vendorLength > 254){  // guard
       Debug_println("vorbis comment too long");
       return 0;
    }
    ...
    for(int i = 0; i < nrOfComments; i++){
        ...
        if(commentLength > 254) {Debug_println("vorbis comment too long"); return 0;}
...
int parseVorbisCodebook(){
    ...
    for(i = 0; i < s_nrOfCodebooks; i++){
        ...
        if(ret) Debug_printf("codebook %i returned err\n", i);
        if(ret) goto err_out;
    }
    ...
    for(; i >= 0; i--){
        ret = bitReader(16);
        if(ret != 0){
            Debug_printf("err while reading backend settings\n");
            goto err_out;
        }
    }
    ...
    for(i = 0; i < s_nrOfFloors; i++) {
        ...
        if(s_floor_type[i] < 0 || s_floor_type[i] >= VI_FLOORB) {
            Debug_println("err while reading floors");
            goto err_out;
        }
        if(s_floor_type[i]){
        ...
        if(!s_floor_param[i]){
            Debug_println("floor parameter not found");
            goto err_out;
        }
    }
    ...
    for(i = 0; i < s_nrOfResidues; i++){
         if(res_unpack(s_residue_param + i)){
            Debug_println("err while unpacking residues");
            goto err_out;
         }
    }
    ...
    for(i = 0; i < s_nrOfMaps; i++) {
        ...
        if(mapping_info_unpack(s_map_param + i)){
            Debug_println("err while unpacking mappings");
            goto err_out;
        }
    }
    ...
    for(i = 0; i < s_nrOfModes; i++) {
        ...
        if(s_mode_param[i].mapping >= s_nrOfMaps){
            Debug_println("too many modes");
            goto err_out;
        }
    }

    if(bitReader(1) != 1){
        Debug_println("codebooks, end bit not found");
        goto err_out;
    }
...
//    vorbis_info_clear(vi);
    Debug_printf("err in codebook!  at pos %d\n", s_bitReader.headptr - s_bitReader.data);
...
int vorbis_book_unpack(codebook_t *s) {
    ...
    if(bitReader(24) != 0x564342){
        Debug_println("String \"BCV\" not found");
        goto _eofout;  // "BCV"
    }
    ...
    if(s->entries == -1) {Debug_println("no entries in unpack codebooks ?");   goto _eofout;}
    ...
    switch(maptype) {
        ...
        case 2:
            ...
            break;
        default:
            Debug_printf("maptype %i schould be 0, 1 or 2\n", maptype);
            goto _errout;
    }
    if(oggpack_eop()) goto _eofout;
...
int8_t bitReader_adv(uint16_t nBits) {
    ...
    if(s_bitReader.headend < 1){
        return -1;
        Debug_println("error in bitreader");
    }
    return 0;
}
//---------------------------------------------------------------------------------------------------------------------
...
int oggpack_eop() {
    if(s_bitReader.headptr -s_bitReader.data  > s_setupHeaderLength){
        Debug_printf("s_bitReader.headptr %i, s_setupHeaderLength %i\n", s_bitReader.headptr, s_setupHeaderLength);
        Debug_println("ogg package 3 overflow");
         return -1;
    }
    return 0;
}
...
int vorbis_dsp_pcmout(int16_t *outBuff, int outBuffSize) {
    if(s_dsp_state->out_begin > -1 && s_dsp_state->out_begin < s_dsp_state->out_end) {
        ...
        if(outBuff) {
            ...
            if(n > outBuffSize) {
                n = outBuffSize;
                Debug_printf("outBufferSize too small, must be min %i (int16_t) words\n", n);
            }
            for(i = 0; i < s_vorbisChannels; i++){
                mdct_unroll_lap(s_blocksizes[0], s_blocksizes[1],
                                s_dsp_state->lW, s_dsp_state->W, (int)s_dsp_state->work[i],
                                (int)s_dsp_state->mdctright[i], _vorbis_window((int)s_blocksizes[0] >> 1),
                                _vorbis_window((int)s_blocksizes[1] >> 1),
