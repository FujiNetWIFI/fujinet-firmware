...
public:
    ...
    bool connecttoFS(FileSystemSDFAT &fs, const char* path, uint32_t resumeFilePos = 0);
...
//+++ W E B S T R E A M  -  H E L P   F U N C T I O N S +++
    ...
    bool     readID3V1Tag();
    bool     streamDetection(uint32_t bytesAvail);
    void     seek_m4a_stsz();
    void     seek_m4a_ilst();
...
private:
    ...
    fnHttpClient          client;       // @suppress("Abstract class cannot be instantiated")
    fnHttpClient          clientsecure; // @suppress("Abstract class cannot be instantiated")
    fnHttpClient*         _client = nullptr;
