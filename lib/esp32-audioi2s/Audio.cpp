...
//    if(psramInit() && m_buffSizePSRAM > 0) {
    if(m_buffSizePSRAM > 0) {
        // PSRAM found, AudioBuffer will be allocated in PSRAM
        m_f_psram = true;
        m_buffSize = m_buffSizePSRAM;
        m_buffer = (uint8_t*) heap_caps_calloc(m_buffSize, sizeof(uint8_t));
...
//    if(psramInit()) m_chbufSize = 4096; else m_chbufSize = 512 + 64;
    ...
    if(!m_chbuf || !m_lastHost || !m_outBuff || !m_ibuff) Debug_println("oom");
...
void Audio::setBufsize(int rambuf_sz, int psrambuf_sz) {
    if(InBuff.isInitialized()) {
        Debug_println("Audio::setBufsize must not be called after audio is initialized");
        return;
    }
...
esp_err_t Audio::i2s_mclk_pin_select(const uint8_t pin) {
    ...
    if(pin != 0 && pin != 1 && pin != 3) {
        Debug_printf("Only support GPIO0/GPIO1/GPIO3, gpio_num:%d\n", pin);
...
void Audio::setDefaults() {
    ...
    AUDIO_INFO("buffers freed, free Heap: %u bytes", fnSystem.get_free_heap_size());
...
bool Audio::connecttomarytts(const char* speech, const char* lang, const char* voice){
    ...
    memcpy(m_lastHost, speech, 256);
    char* speechBuff = (char*)malloc(speechBuffLen);
    if(!speechBuff) {Debug_println("out of memory");
...
void Audio::urlencode(char* buff, uint16_t buffLen, bool spacesOnly) {
    ...
    if(!tmpbuff) {Debug_println("out of memory"); return;}
    char c;
    char code0;
    char code1;
    uint16_t j = 0;
...
size_t Audio::readAudioHeader(uint32_t bytes){
    ...
    if(!isRunning()){
        Debug_println("Processing stopped due to invalid audio header");
        return 0;
    }
    return bytesReaded;
}
...
int Audio::read_FLAC_Header(uint8_t *data, size_t len) {
    ...
    if(m_controlCounter == FLAC_MAGIC) { /* check MAGIC STRING */
        ...
        if(specialIndexOf(data, "fLaC", 10) != 0) {
            Debug_println("Magic String 'fLaC' not found in header");
            stopSong();
            return -1;
        }
    ...
    if(m_controlCounter == FLAC_SINFO) { /* Stream info block */
        ...
        if(m_flacMaxFrameSize > InBuff.getMaxBlockSize()) {
            Debug_println("FLAC maxFrameSize too large!");
            stopSong();
            return -1;
        }
//        InBuff.changeMaxBlockSize(m_flacMaxFrameSize);
        ...
        if((bps != 8) && (bps != 16)){
            Debug_printf("bits per sample must be 8 or 16, is %i\n", bps);
            stopSong();
            return -1;
        }
...
int Audio::read_ID3_Header(uint8_t *data, size_t len) {
    ...
    if(m_controlCounter == 4){  // get the frame size
        ...
        if(compressed){
            if(m_f_Log) Debug_println("iscompressed");
            decompsize = bigEndian(data + 6, 4);
            remainingHeaderBytes -= 4;
            (void) decompsize;
            if(m_f_Log) Debug_printf("decompsize=%u\n", decompsize);
            return 6 + 4;
        }
        return 6;
    }
    ...
    if(m_controlCounter == 10){ // frames in V2.2, 3bytes identifier, 3bytes size descriptor
        ...
        if(startsWith(tag, "PIC")) { // image embedded in header
            if(getDatamode() == AUDIO_LOCALFILE){
                ...
                if(m_f_Log) Debug_printf("Attached picture seen at pos %d length %d\n", APIC_pos, APIC_size);
            }
        }
            ...
            if(getDatamode() == AUDIO_LOCALFILE){
                ...
                if(m_f_Log) Debug_printf("Attached lyrics seen at pos %d length %d\n", SYLT_pos, SYLT_size);
            }
        }
        else{
...
/*
    ...
    if(m_controlCounter == M4A_FTYP) { /* check_m4a_file */
        ...
        if(specialIndexOf(data, "ftyp", 10) != 4) {
            Debug_println("atom 'ftyp' not found in header");
            stopSong();
            return -1;
        }
        ...
        if((m4a !=8) && (isom != 8) && (mp42 != 8)){
            Debug_printf("subtype 'MA4 ', 'isom' or 'mp42' expected, but found '%s '\n", (data + 8));
            stopSong();
            return -1;
        }
    ...
    if(m_controlCounter == M4A_CHK) { /* check  Tag */
        ...
        if(specialIndexOf(data, "moov", 10) == 4) {
            ...
            if(m_f_Log) Debug_printf("atom %s found\n", atomName);
    ...
    if(m_controlCounter == M4A_TRAK) {  // trak
        if(specialIndexOf(data, "esds", len) > 0){
            ...
            if(streamType!= 5) { Debug_println("Streamtype is not audio!"); }
            ...
            if(chConfig >  2) { Debug_println("Channel Configurations with more than 2 channels is not allowed!"); }
...
size_t Audio::process_m3u8_ID3_Header(uint8_t* packet){
    ...
    if(specialIndexOf(packet, "ID3", 4) != 0) { // ID3 not found
        if(m_f_Log) Debug_println("m3u8 file has no mp3 tag");
    ...
    id3Size += 10;
    if(m_f_Log) Debug_printf("ID3 framesSize: %i\n", id3Size);
    if(m_f_Log) Debug_printf("ID3 version: 2.%i\n", ID3version);

    if(m_f_exthdr) {
        Debug_println("ID3 extended header in m3u8 files not supported");
        return 0;
    }
    if(m_f_Log)
        Debug_println("ID3 normal frames");

    if(specialIndexOf(&packet[10], "PRIV", 5) != 0) { // tag PRIV not found
        Debug_println("tag PRIV in m3u8 Id3 Header not found");
        return 0;
    }
...
uint32_t Audio::stopSong() {
    ...
    if(audiofile){
        ...
        audiofile.close();
        AUDIO_INFO("Closing audio file");
        Debug_println()"Closing audio file");  // for debug
    }
...
bool Audio::playChunk() {
    ...
    Debug_println("BitsPer Sample must be 8 or 16!");
    m_validSamples = 0;
    stopSong();
    return false;
}
    ...
    if(m_playlistFormat != FORMAT_M3U8){ // normal process
        ...
        switch(getDatamode()){
            ...
            case AUDIO_PLAYLISTDATA:
                ...
                if(host){
                    f_noNewHost = false;
                    timestamp1 = fnSystem.millis();
                    httpPrint(host);
                }
                else {
                    f_noNewHost = true;
                    timestamp2 = fnSystem.millis() + remaintime;
            ...
            case AUDIO_DATA:
                ...
                if(f_noNewHost){
                    ...
                    if(m_f_continue){ // processWebStream() needs more data
                        remaintime = (int32_t)(m_m3u8_targetDuration * 1000) - (fnSystem.millis() - timestamp1);
...
//    if(m_playlistFormat == FORMAT_M3U8 && !psramFound()){Debug_println("m3u8 playlists requires PSRAM enabled!");}
    vector_clear_and_shrink(m_playlistContent);
    while(true){  // outer while

        uint32_t ctime = fnSystem.millis();
        ...
        while(true) { // inner while
            ...
            if(ctime + timeout < fnSystem.millis()) {
                Debug_println("timeout");
                for(int i = 0; i<m_playlistContent.size(); i++) Debug_printf("pl%i = %s\n", i, m_playlistContent[i]);
                goto exit;}
        } // inner while
        ...
        if(m_playlistContent.size() == 100){
            if(m_f_Log) Debug_println("the maximum number of lines in the playlist has been reached");
            break;
        }
    ...
    for(int i= 0; i < lines; i++){
        ...
        if(p1 >= 0 && p2 > p1){                                     // #196 set all between "< ...> to lowercase
            for(uint8_t j = p1; j < p2; j++){
                m_playlistContent[i][j] = strlwr(m_playlistContent[i][j]);
            }
        }
...
const char* Audio::parsePlaylist_M3U8(){
    ...
    if(lines){
        for(int i= 0; i < lines; i++){
            ...
            if(startsWith(m_playlistContent[i],"#EXT-X-STREAM-INF:")){
                ...
                if((!endsWith(m_playlistContent[i+1], "m3u8" ) && indexOf(m_playlistContent[i+1], "m3u8?") == -1)){
                    ...
                    if(pos < 0){ // not found
                        ...
                        if(pos1 < 0) pos1 = 0;
                        Debug_printf"codec %s in m3u8 playlist not supported\n", m_playlistContent[i] + pos1);
                        goto exit;
                    }
                }
                i++;                                                    // next line
            ...
            if(m_f_Log) Debug_printf("m_m3u8_targetDuration %d\n", m_m3u8_targetDuration);
                ...
                if(m_hashQueue.size() == 0){
                    ...
                    for(int i = 0; i< m_hashQueue.size(); i++){
                        if(hash == m_hashQueue[i]){
                            if(m_f_Log) Debug_printf("file already known %s\n", tmp);
                            known = true;
                        }
                    }
                    if(!known){
    ...
    if(m_playlistURL.size() > 0){
        ...
        if(m_f_Log) Debug_printf("now playing %s\n", m_playlistBuff);
...
void Audio::processLocalFile() {
    ...
    if(!f_stream){
        if(m_controlCounter != 100) {
        	if((fnSystem.millis() - ctime) > timeout) {
        		Debug_println("audioHeader reading timeout");
        		m_f_running = false;
        		return;
        	}
            ...
            if(m_f_Log) Debug_printf("m_audioDataStart %d\n", m_audioDataStart);
        }
    }
    ...
    if(m_resumeFilePos){
        ...
        if(m_f_Log){
            Debug_printf("m_resumeFilePos %d\n", m_resumeFilePos);
            Debug_printf("m_audioDataStart %d\n", m_audioDataStart);
            Debug_printf("m_audioCurrentTime %f\n", (double)m_audioCurrentTime);
            Debug_printf("m_file_size %d\n", m_file_size);
        }
        m_resumeFilePos = 0;
        f_stream = false;
    }
...
void Audio::processWebFile() {
    ...
    if(!m_contentlength && !m_f_tts) {Debug_println("webfile without contentlength!"); stopSong(); return;} // guard
    ...
    if(InBuff.bufferFilled() > maxFrameSize && !f_stream) {  // waiting for buffer filled
        f_stream = true;  // ready to play the audio data
        uint16_t filltime = fnSystem.millis() - m_t0;
        ...
        if(res > 0){
            ...
            if(f_firstPacket){  // search for ID3 Header in the first packet
                ...
                if(ID3_HeaderSize > ts_packetsize){
                    Debug_println("ID3 Header is too big");
                    stopSong();
                    return;
                }
                if(ID3_HeaderSize){
            ...
            if(byteCounter > m_contentlength) Debug_println("byteCounter overflow");
        }
    ...
    if(availableBytes){ // an ID3 header could come here
        ...
        if(firstBytes){
            ...
            if(m_controlCounter < 100){
                ...
                if(ID3ReadPtr > ID3BuffSize) {Debug_println("buffer overflow"); stopSong(); return;}
                return;
            }
...
void Audio::playAudioData(){
    ...
    if(bytesDecoded < 0) {  // no syncword found or decode error, try next chunk
        Debug_printf("err bytesDecoded %i\n", bytesDecoded);
        uint8_t next = 200;
...
bool Audio::parseHttpResponseHeader() { // this is the response to a GET / request
    ...
    while(true){  // outer while
        ...
        while(_client->available()){
            ...
            if(pos == 510){
                rhl[pos] = '\0';
                if(m_f_Log) Debug_println("responseHeaderline overflow");
            }
        } // inner while

        if(!pos){vTaskDelay(3); continue;}

        if(m_f_Log) {Debug_printf("httpResponseHeader: %s\n", rhl);}
    ...
    lastToDo:
        if(m_codec != CODEC_NONE){
            ...
            if(m_f_Log) {Debug_printf("Switch to DATA, metaint is %d\n", m_metaint);}
            ...
            if(m_f_Log) {Debug_println("now parse playlist");}
        }
        else{
...
bool Audio:: initializeDecoder(){
    ...
    switch(m_codec){
        case CODEC_MP3:
            ...
            gfH = fnSystem.get_free_heap_size();
...
bool Audio::parseContentType(char* ct) {
    ...
    switch(ct_val){
        case CT_MP3:
            m_codec = CODEC_MP3;
            if(m_f_Log) { Debug_printf("ContentType %s, format is mp3\n", ct); } //ok is likely mp3
            break;
        case CT_AAC:
            m_codec = CODEC_AAC;
            if(m_f_Log) { Debug_printf("ContentType %s, format is aac\n", ct); }
            break;
        case CT_M4A:
            m_codec = CODEC_M4A;
            if(m_f_Log) { Debug_printf("ContentType %s, format is aac\n", ct); }
        ...
        case CT_FLAC:
            m_codec = CODEC_FLAC;
            if(m_f_Log) { Debug_printf("ContentType %s, format is flac\n", ct); }
            break;
        case CT_OPUS:
            m_codec = CODEC_OPUS;
            if(m_f_Log) { Debug_printf("ContentType %s, format is opus\n", ct); }
            break;
        case CT_VORBIS:
            m_codec = CODEC_VORBIS;
            Debug_printf("ContentType %s, format is vorbis\n", ct);
            break;
        case CT_WAV:
            m_codec = CODEC_WAV;
            if(m_f_Log) { Debug_printf"ContentType %s, format is wav\n", ct); }
            break;
        case CT_OGG:
        ...
        case CT_TXT: // overwrite text/plain
            if(m_expectedCodec == CODEC_AAC){ m_codec = CODEC_AAC; if(m_f_Log) Debug_println("set ct from M3U8 to AAC");}
            if(m_expectedCodec == CODEC_MP3){ m_codec = CODEC_MP3; if(m_f_Log) Debug_println("set ct from M3U8 to MP3");}

            if(m_expectedPlsFmt == FORMAT_ASX){ m_playlistFormat = FORMAT_ASX;  if(m_f_Log) Debug_println("set playlist format to ASX");}
            if(m_expectedPlsFmt == FORMAT_M3U){ m_playlistFormat = FORMAT_M3U;  if(m_f_Log) Debug_println("set playlist format to M3U");}
            if(m_expectedPlsFmt == FORMAT_M3U8){m_playlistFormat = FORMAT_M3U8; if(m_f_Log) Debug_println("set playlist format to M3U8");}
            if(m_expectedPlsFmt == FORMAT_PLS){ m_playlistFormat = FORMAT_PLS;  if(m_f_Log) Debug_println("set playlist format to PLS");}
            break;
        default:
...
int Audio::sendBytes(uint8_t* data, size_t len) {
    ...
    switch(m_codec){
        ...
        default: {Debug_println("no valid codec found codec = %d\n", m_codec); stopSong();}
    }
    ...
    // status: bytesDecoded > 0 and m_decodeError >= 0
    {
        ...
        if(f_setDecodeParamsOnce){
            ...
            setDecoderItems();
            m_PlayingStartTime = fnSystem.millis();
        }
    }
...
void Audio::compute_audioCurrentTime(int bd) {
    ...
    static bool f_CBR = true; // constant bitrate
...
uint32_t Audio::getTotalPlayingTime() {
    ...
    return fnSystem.millis() - m_PlayingStartTime;
}
//---------------------------------------------------------------------------------------------------------------------
...
void Audio::setI2SCommFMT_LSB(bool commFMT) {
    ...
    if (commFMT) {
        if(m_f_Log) Debug_println("commFMT LSB");
...
bool Audio::playSample(int16_t sample[2]) {
    ...
    if(err != ESP_OK) {
        Debug_printf("ESP32 Errorcode %i\n", err);
        return false;
    }
    if(m_i2s_bytesWritten < 4) {
        Debug_println("Can't stuff any more in I2S..."); // increase waitingtime or outputbuffer
        return false;
    }
    return true;
}
...
bool Audio::ts_parsePacket(uint8_t* packet, uint8_t* packetStart, uint8_t* packetLength) {
    ...
    if(packet == NULL){
        if(m_f_Log) Debug_println("parseTS reset");
    ...
    if(packet[0] != 0x47) {
        Debug_printf("ts SyncByte not found, first bytes are %X %X %X %X\n", packet[0], packet[1], packet[2], packet[3]);
        stopSong();
        return false;
    }
    int PID = (packet[1] & 0x1F) << 8 | (packet[2] & 0xFF);
    if(m_f_Log) Debug_printf("PID: 0x%04X(%d)\n", PID, PID);
    int PUSI = (packet[1] & 0x40) >> 6;
    if(m_f_Log) lDebug_printf("Payload Unit Start Indicator: %d\n", PUSI);
    int AFC = (packet[3] & 0x30) >> 4;
    if(m_f_Log) Debug_printf("Adaption Field Control: %d\n", AFC);

    int AFL = -1;
    if((AFC & 0b10) == 0b10) {  // AFC '11' Adaptation Field followed
        AFL = packet[4] & 0xFF; // Adaptation Field Length
        if(m_f_Log) Debug_printf("Adaptation Field Length: %d\n", AFL);
    }
    ...
    if(PID == 0) {
        ...
        if(m_f_Log) Debug_printf("Section Length: %d\n", sectionLength);
        ...
        for(int i = startOfProgramNums; i <= sectionLength; i += lengthOfPATValue) {
            ...
            if(m_f_Log) Debug_printf("Program Num: 0x%04X(%d) PMT PID: 0x%04X(%d)\n", program_number, program_number,
        ...
        *packetLength = 0;
        ...
        if(m_f_Log) Debug_println("AAC");
        uint8_t posOfPacketStart = 4;
        if(AFL >= 0) {posOfPacketStart = 5 + AFL;
        if(m_f_Log) Debug_printf("posOfPacketStart: %d\n", posOfPacketStart);}
        // Packetized Elementary Stream (PES) - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        if(m_f_Log) Debug_printf("PES_DataLength %i\n", PES_DataLength);
        if (PES_DataLength > 0) {
            ...
            *packetLength   = TS_PACKET_SIZE - posOfPacketStart - fillData;
            ...
            if(m_f_Log) Debug_printf("First 3 bytes: %02X %02X %02X\n", firstByte, secondByte, thirdByte);
            if(firstByte == 0x00 && secondByte == 0x00 && thirdByte == 0x01) { // Packet start code prefix
                ...
                if(StreamID >= 0xE0 && StreamID <= 0xEF) {Debug_println("video stream!"); return false;}
                uint8_t PES_HeaderDataLength = packet[posOfPacketStart + 8] & 0xFF;
                if(m_f_Log) Debug_printf("PES_headerDataLength %d\n", PES_HeaderDataLength);
                int PES_PacketLength =
                    ((packet[posOfPacketStart + 4] & 0xFF) << 8) + (packet[posOfPacketStart + 5] & 0xFF);
                if(m_f_Log) Debug_printf("PES Packet length: %d\n", PES_PacketLength);
                ...
                if(posOfPacketStart + startOfData >= 188){ // only fillers in packet
                    if(m_f_Log) Debug_printf("posOfPacketStart + startOfData %i\n", posOfPacketStart + startOfData);
                    *packetStart = 0;
                    *packetLength = 0;
                    PES_DataLength -= (PES_HeaderDataLength + 3);
                    fillData = (posOfPacketStart + startOfData) - 188;
                    if(m_f_Log) Debug_printf("fillData %i\n", fillData);
                    return true;
                }
                if(m_f_Log) Debug_printf("First AAC data byte: %02X\n", packet[posOfPacketStart + startOfData]);
                if(m_f_Log) Debug_printf("Second AAC data byte: %02X\n", packet[posOfPacketStart + startOfData + 1]);
        ...
        for(int i = 0; i < pidsOfPMT.number; i++) {
            if(PID == pidsOfPMT.pids[i]) {
                ...
                while(cursor < sectionLength - 1) {
                    ...
                    if(m_f_Log) Debug_printf("Stream Type: 0x%02X Elementary PID: 0x%04X\n", streamType, elementaryPID);

                    if(streamType == 0x0F || streamType == 0x11) {
                        if(m_f_Log) Debug_println("AAC PID discover");
                        pidOfAAC= elementaryPID;
                    }
                    int esInfoLength = ((packet[PLS + cursor + 3] & 0x0F) << 8) | (packet[PLS + cursor + 4] & 0xFF);
                    if(m_f_Log) Debug_printf("ES Info Length: 0x%04X\n", esInfoLength);
...
size_t Audio::chunkedDataTransfer(uint8_t* bytes){
    ...
    if(m_f_Log) Debug_printf("chunksize %d\n", chunksize);
    *bytes = byteCounter;
    return chunksize;
}
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
bool Audio::streamDetection(uint32_t bytesAvail){
    static uint32_t tmr_slow = fnSystem.millis();
    static uint32_t tmr_lost = fnSystem.millis();
    ...
    if(tmr_lost < fnSystem.millis()){
        cnt_lost++;
        tmr_lost = fnSystem.millis() + 1000;
...
void Audio::seek_m4a_ilst(){
 ...
 //           log_i("name %s pos %d, size %d", tmp.name, tmp.pos, tmp.size);
        ...
        if(!found){
            Debug_println("m4a atom ilst not found");
            audiofile.seek(0);
            return;
        }
...
void Audio::seek_m4a_stsz(){
    ...
    for(int i = 0; i< 6; i++){
        ...
        while(seekpos < at.pos + at.size){
            ...
            seekpos += tmp.size;
            if(strcmp(tmp.name, name[i]) == 0) {memcpy((void*)&at, (void*)&tmp, sizeof(tmp)); found = true;}
            if(m_f_Log) Debug_printf("name %s pos %d, size %d\n", tmp.name, tmp.pos, tmp.size);
        }
    ...
    if(m_f_Log) Debug_printf("number of entries in stsz: %d\n", m_stsz_numEntries);
...
noSuccess:
    m_stsz_numEntries= 0;
    m_stsz_position = 0;
    Debug_println("m4a atom stsz not found");
    audiofile.seek(0);
    return;
}
//----------------------------------------------------------------------------------------------------------------------
...
uint32_t Audio::flac_correctResumeFilePos(uint32_t resumeFilePos){
    ...
    while(!found || pos == m_file_size){
        if(p1 == 0xFF && p2 == 0xF8){found = true; Debug_println("found"); break;}
        p1 = p2;
        p2 = audiofile.read();
        pos++;
    }
...
uint8_t Audio::determineOggCodec(uint8_t* data, uint16_t len){
    ...
    if(idx >= 0){Debug_println("vorbis"); return CODEC_VORBIS;}
    return CODEC_NONE;
}
//----------------------------------------------------------------------------------------------------------------------
