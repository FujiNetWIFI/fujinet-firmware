...
bool OPUSDecoder_AllocateBuffers(){
    ...
    if(!CELTDecoder_AllocateBuffers()) {Debug_println("CELT not init"); return false;}
    s_opusSegmentTable = (uint16_t*)malloc(256 * sizeof(uint16_t));
    if(!s_opusSegmentTable) {Debug_println("CELT not init"); return false;}
    CELTDecoder_ClearBuffer();
    OPUSDecoder_ClearBuffers();
    s_opusError = celt_decoder_init(2); if(s_opusError < 0) {Debug_println("CELT not init"); return false;}
    s_opusError = celt_decoder_ctl(CELT_SET_SIGNALLING_REQUEST,  0); if(s_opusError < 0) {Debug_printf("CELT not init"); return false;}
    s_opusError = celt_decoder_ctl(CELT_SET_END_BAND_REQUEST,   21); if(s_opusError < 0) {Debug_printf("CELT not init"); return false;}
    OPUSsetDefaults();
    return true;
}
