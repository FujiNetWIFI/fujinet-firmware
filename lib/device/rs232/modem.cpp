...
static void _telnet_event_handler(telnet_t *telnet, telnet_event_t *ev, void *user_data)
{
    ...
    case TELNET_EV_DATA:
        if (ev->data.size)
            modem->rx(ev->data.buffer, ev->data.size);
        break;
    case TELNET_EV_SEND:
...
rs232Modem::rs232Modem(FileSystem *_fs, bool snifferEnable)
{
    ...
    activeFS = _fs;
    modemBaud = SYSTEM_BUS.getBaudrate();
...
// 0x57 / 'W' - WRITE
void rs232Modem::rs232_write(const std::optional<ByteBuffer>& data)
{
    ...
    {
        cmdOutput = false;
        cmd.assign(data->begin(), data->end());
...
void rs232Modem::tx(ByteBuffer data)
{
    rs232_write(data);
}
...
{
    rx(data.data(), data.size());
}

void rs232Modem::rx(int n)
{
    uint8_t b = n;
    rx(&b, 1);
}

void rs232Modem::rx(const char *s)
{
    rx(s, strlen(s));
}
