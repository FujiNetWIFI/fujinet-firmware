...
{
    Debug_printf("NetworkProtocolSSH::ctor\n");
}
...
{
    Debug_printf("NetworkProtocolSSH::dtor\n");
}
...
bool NetworkProtocolSSH::open(EdUrlParser *urlParser, cmdFrame_t *cmdFrame)
{
    ...
    Debug_printf("NetworkProtocolSSH::open(%s:%s)", urlParser->hostName.c_str(), urlParser->port.c_str());
...
{
    Debug_printf("NetworkProtocolSSH::close()\n");
...
bool NetworkProtocolSSH::read(unsigned short len)
{
    ...
    Debug_printf("NetworkProtocolSSH::read(%u)\n", len);
...
bool NetworkProtocolSSH::write(unsigned short len)
{
    ...
    Debug_printf("NetworkProtocolSSH::write(%u)\n", len);
...
void NetworkProtocolSSH::status_client(NetworkStatus *status)
{
...
void NetworkProtocolSSH::status_server(NetworkStatus *status)
{
    if (client.connected())
        status_client(status);
    else
...
{
    Debug_printf("NetworkProtocolSSH::special_inquiry(%02x)\n",cmd);
 ...
 * @return error flag. TRUE on error. FALSE on success.
 */
bool NetworkProtocolSSH::open_server(unsigned short port)
{
 ...
 */
bool NetworkProtocolSSH::open_client(string hostname, unsigned short port)
{
    int res = 0;
 ...
 */
bool NetworkProtocolSSH::special_accept_connection()
{
    if (server == nullptr)
    {
