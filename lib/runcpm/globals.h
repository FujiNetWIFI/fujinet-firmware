...
/* The engine is compiled exactly once (runcpm_core.cpp) with normal external
 * linkage, so RunCPM symbols are plain (non-static). */
#define RUNCPM_DECL

/* Forward declarations to prevent precedence compilation errors inside the
 * RunCPM header chain. */
