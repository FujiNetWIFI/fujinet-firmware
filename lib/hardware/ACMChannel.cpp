...
void SerialACM::begin()
{
    ...
    while (true)
    {
        ...
        Debug_printv("Opening CDC ACM device 0x%04X:0x%04X...", USB_DEVICE_VID,
                     USB_DEVICE_PID);
        esp_err_t err =
            cdc_acm_host_open(USB_DEVICE_VID, USB_DEVICE_PID, 0, &dev_config, &cdc_dev);
        if (ESP_OK != err)
        {
#if 0
        ...
        ESP_ERROR_CHECK(cdc_acm_host_data_tx_blocking(cdc_dev, (const uint8_t *) TX_STRING,
                                                      strlen(TX_STRING), TX_TIMEOUT_MS));
#endif
        vTaskDelay(pdMS_TO_TICKS(100));
        ...
        Debug_printv("Line Get: Rate: %" PRIu32 ", Stop bits: %" PRIu8 ", Parity: %" PRIu8
                     ", Databits: %" PRIu8 "",
        ...
        Debug_printv("Line Set: Rate: %" PRIu32 ", Stop bits: %" PRIu8 ", Parity: %" PRIu8
                     ", Databits: %" PRIu8 "",
                     line_coding.dwDTERate, line_coding.bCharFormat, line_coding.bParityType,
                     line_coding.bDataBits);
        ...
        Debug_printv("Example finished successfully!"
                     " You can reconnect the device to run again.");
...
{
    cdc_acm_host_data_tx_blocking(cdc_dev, (const uint8_t *) buffer, length, TX_TIMEOUT_MS);
    return length;
}
