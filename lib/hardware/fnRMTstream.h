...
struct fn_rmt_config_t
{
    ...
    union
    {
        fn_rmt_tx_config_t tx_config; /*!< RMT TX parameter */
        // REMOVE
        fn_rmt_rx_config_t rx_config; /*!< RMT RX parameter */
    };
} ;
 ...
 *
 */
typedef intr_handle_t fn_rmt_isr_handle_t;
 ...
 *
 */
typedef void (*fn_rmt_tx_end_fn_t)(rmt_channel_t channel, void *arg);

// REMOVE
/**
 * @brief Structure encapsulating a RMT TX end callback
 */
struct fn_rmt_tx_end_callback_t
{
    fn_rmt_tx_end_fn_t function; /*!< Function which is called on RMT TX end */
    void *arg;                /*!< Optional argument passed to function */
} ;

// REMOVE - probably - the ISR will call a "get next bit" function instead
/**
...
{
private:
    void rmt_set_tx_wrap_en(bool en);
    void rmt_set_data_mode(rmt_data_mode_t data_mode);
...
public:
     ...
     *     - ESP_OK Success
     */
    esp_err_t rmt_config(const fn_rmt_config_t *rmt_param);
     ...
     */
    esp_err_t rmt_isr_register(void (*fn)(void *), void *arg, int intr_alloc_flags, fn_rmt_isr_handle_t *handle);
     ...
     */
    esp_err_t rmt_isr_deregister(fn_rmt_isr_handle_t handle);

    void rmt_fill_memory(rmt_channel_t channel, const rmt_item32_t* item, uint16_t item_num, uint16_t mem_offset);
     ...
     *     - ESP_OK Init success.
     */
    esp_err_t rmt_translator_init(rmt_channel_t channel, fn_sample_to_rmt_t fn);
     ...
     */
    fn_rmt_tx_end_callback_t rmt_register_tx_end_callback(fn_rmt_tx_end_fn_t function, void *arg);
