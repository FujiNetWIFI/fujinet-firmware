...
sftp_reply_status(sftp_client_message msg, uint32_t status, const char *message)
{
    ...
    SSH_LOG(SSH_LOG_PROTOCOL, "Sending status %lu, message: %s", status,
...
process_open(sftp_client_message client_msg)
{
    ...
    SSH_LOG(SSH_LOG_PROTOCOL, "Processing open: filename %s, mode=0%lu",
            filename, mode);
...
#ifdef ESP_PLATFORM
            /* lstat not available on ESP-IDF, use stat instead */
...
process_mkdir(sftp_client_message client_msg)
{
    ...
    SSH_LOG(SSH_LOG_PROTOCOL, "Processing mkdir %s, mode=0%lu",
            filename, mode);
...
{
#ifdef ESP_PLATFORM
    /* statvfs not available on ESP-IDF */
    SSH_LOG(SSH_LOG_PROTOCOL, "statvfs not supported on ESP-IDF");
    sftp_reply_status(client_msg, SSH_FX_OP_UNSUPPORTED, "statvfs not supported on this platform");
    return SSH_ERROR;
#else
