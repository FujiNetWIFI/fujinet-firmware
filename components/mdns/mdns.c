    ...
    if (header.questions && !parsed_packet->questions && !parsed_packet->discovery && !header.answers) {
        ...
        while (content < (data + len)) {
            ...
            if (type == MDNS_TYPE_PTR) {
                ...
                if (search_result) {
                    // For PTR searches, construct the instance name from the answer
                    char instance_name[256] = {0};
                    if (name->proto[0]) {
                        // Full service type: service.proto (after parsing shift)
                        strlcpy(instance_name, name->service, sizeof(instance_name));
                        strlcat(instance_name, ".", sizeof(instance_name));
                        strlcat(instance_name, name->proto, sizeof(instance_name));
                    } else if (name->service[0]) {
                        // service.proto without shift
                        strlcpy(instance_name, name->host, sizeof(instance_name));
                        strlcat(instance_name, ".", sizeof(instance_name));
                        strlcat(instance_name, name->service, sizeof(instance_name));
                    } else {
                        // Just host
                        strlcpy(instance_name, name->host, sizeof(instance_name));
                    }
                    _mdns_search_result_add_ptr(search_result, instance_name, search_result->service, search_result->proto,
...
static mdns_search_once_t *_mdns_search_find_from(mdns_search_once_t *s, mdns_name_t *name, uint16_t type, mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol)
{
    ...
    while (s) {
        ...
        if (type == MDNS_TYPE_PTR && type == s->type) {
            // For PTR records, reconstruct the full service name
            char full_service[MDNS_NAME_BUF_LEN * 2] = {0};
            if (name->host[0]) {
                strlcpy(full_service, name->host, sizeof(full_service));
                strlcat(full_service, ".", sizeof(full_service));
                strlcat(full_service, name->service, sizeof(full_service));
            } else {
                strlcpy(full_service, name->service, sizeof(full_service));
            }
            if (!strcasecmp(full_service, s->service) && !strcasecmp(name->proto, s->proto)) {
                return s;
            }
        }
