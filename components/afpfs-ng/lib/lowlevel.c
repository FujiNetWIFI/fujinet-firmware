...
#ifdef __linux__
	fp->sync = (flags & (O_SYNC | O_DIRECT)) ? 1 : 0;  
#else
	fp->sync = (flags & O_SYNC) ? 1 : 0;  
#endif
...
/* FIXME: check AFP version */
	...
	if (volume->server->using_version->av_number<30) {
		for (p=filebase; p; p=p->next) {
			unsigned int temp_perms = p->unixprivs.permissions;
			set_nonunix_perms(&temp_perms, p);
			p->unixprivs.permissions = temp_perms;
		}
	}
	...
	int resource)
{
	...
	unsigned int temp_uid = stbuf->st_uid;
	unsigned int temp_gid = stbuf->st_gid;
	if (translate_uidgid_to_client(volume, &temp_uid, &temp_gid)) {
		return -EIO;
	}
	stbuf->st_uid = temp_uid;
	stbuf->st_gid = temp_gid;
