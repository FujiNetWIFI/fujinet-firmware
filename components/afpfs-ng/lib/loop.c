...
int afp_main_quick_startup(pthread_t * thread)
{
	...
	/* The default ESP32 pthread stack (3072 bytes) is too small:
	 * dsi_recv + signal handling overflows it, corrupting DSI reply buffers.
	 * 6144 bytes is sufficient for the pselect/recv event loop.
	 * Note: pthread_attr_setstack is not implemented on ESP32 (stub), so we
	 * cannot place the stack in PSRAM; setstacksize allocates from internal RAM.
	 * Hardware MPI must be disabled (CONFIG_MBEDTLS_HARDWARE_MPI not set) so
	 * that mbedtls_mpi_exp_mod uses software bignum and does not require
	 * DMA-accessible (internal-RAM) operand buffers. */
