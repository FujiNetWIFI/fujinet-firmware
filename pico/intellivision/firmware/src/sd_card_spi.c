...
static inline uint32_t sd_lba_to_addr(sd_card_t *sd_card_p, uint32_t lba) {
    if (sd_card_p->state.card_type == SDCARD_V2HC) {
        return lba;
    }
    return lba * sd_block_size;
}
...
static block_dev_err_t in_sd_read_blocks(sd_card_t *sd_card_p, uint8_t *buffer,
    ...
    if (num_rd_blks == 1)
        status = sd_cmd(sd_card_p, CMD17_READ_SINGLE_BLOCK, sd_lba_to_addr(sd_card_p, data_address), false, 0);
    else
        status = sd_cmd(sd_card_p, CMD18_READ_MULTIPLE_BLOCK, sd_lba_to_addr(sd_card_p, data_address), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
                                          ...
                                          uint32_t * const num_wrt_blks_p)
{
    ...
    status = sd_cmd(sd_card_p, CMD25_WRITE_MULTIPLE_BLOCK, sd_lba_to_addr(sd_card_p, *data_address_p), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
                                          ...
                                          uint32_t const address)
{
    ...
    status = sd_cmd(sd_card_p, CMD24_WRITE_BLOCK, sd_lba_to_addr(sd_card_p, address), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
...
#else
    DBG_PRINTF("SD card type: %s\n",
               sd_card_p->state.card_type == SDCARD_V2HC ? "SDHC/SDXC" :
               sd_card_p->state.card_type == SDCARD_V2   ? "SDSC v2"   :
               sd_card_p->state.card_type == SDCARD_V1   ? "SDSC v1"   :
                                                           "unknown");
#endif
