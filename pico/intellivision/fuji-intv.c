...
#define PINS_TO_SAMPLE 32 // also change in bus_decode.pio !
...
void init_cp1600_pio(PIO pioR, uint sm_rx, PIO pioT, uint sm_tx)
{
    // --- Load programs ---
    uint offset_rx = pio_add_program(pioR, &cp1600_bus_rx_program);
    uint offset_tx = pio_add_program(pioT, &cp1600_bus_tx_program);
    ...
    sm_config_set_in_shift(&c_rx, true, true, PINS_TO_SAMPLE);
    pio_sm_init(pioR, sm_rx, offset_rx, &c_rx);
    pio_sm_set_enabled(pioR, sm_rx, true);
    ...
    sm_config_set_out_shift(&c_tx, true, true, 16);
    pio_sm_init(pioT, sm_tx, offset_tx, &c_tx);
    pio_sm_set_enabled(pioT, sm_tx, true);
}
...
bool __time_critical_func(read_cp1600)(PIO pioR, uint sm_rx, uint32_t *value)
{
    if (!pio_sm_is_rx_fifo_empty(pioR, sm_rx))
    {
        *value = pio_sm_get(pioR, sm_rx);
        return true;
    }
    return false;
}
...
bool __time_critical_func(write_cp1600)(PIO pioT, uint sm_tx, uint16_t data)
{
    if (!pio_sm_is_tx_fifo_full(pioT, sm_tx))
    {
        pio_sm_put(pioT, sm_tx, 0xFFFF); // pindirs to output.
        pio_sm_put(pioT, sm_tx, data);
        return true;
    }
    return false;
}
...
void wait_for_msync(void)
{
    while (gpio_get(PIN_MSYNC))
        ;
}
...
int main()
{
    ...
    init_cp1600_pio(pioR, sm_rx, pioT, sm_tx);
    int i = 128;

    while(1) 
    {
      write_cp1600(pioT, sm_tx, 0xDEAD);
    }

    // while (1)
    // {
    //     uint32_t bus_value;
    //     if (read_cp1600(pioR, sm_rx, &bus_value)) // BDIR = 0
    //     {
    ...
    //     else
    //     {
    //       write_cp1600(pioT, sm_tx, 0xAA55);
    //     }
    // }
}
