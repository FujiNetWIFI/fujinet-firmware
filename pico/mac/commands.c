...
enum latch_bits {
    DIRTN = 0,      // !DIRTN	    This signal sets the direction of head motion. A zero sets direction toward the center of the disk and a one sets direction towards outer edge. When IENBL is high IDIRTN is set to zero. Change of !DIRTN command is not allowed during head movement nor head settlying time.
    STEP,           // !STEP	    At the falling edge of this signal the destination track counter is counted up or down depending on the ID'IIRTN level. After the destination counter in the drive received the falling edge of !STEP, the drive sets !STEP to high.
    MOTORON,        // !MOTORON	    When this signal is set to low, the disk motor is turned on. When IENBL is high, /MOTORON is set to high.
    EJECT,          // EJECT        At the rising edge of the LSTRB, EJECT is set to high and the ejection operation starts. EJECT is set to low at rising edge of ICSTIN or 2 sec maximum after rising edge of EJECT. When power is turned on, EJECT is set to low.
    DATA0,          //              read data - not from latch but comes from RMT device
    na0101,         //              not assigned in latch
    SINGLESIDE,     // !SINGLESIDE  A status bit which is read as one for double sided drive.
    DRVIN,          // !DRVIN       This status bit is read as a zero only if the selected drive is connected to the host computer.
    CSTIN,          // !CSTIN       This status bit is read as a zero only when a diskette is in the drive or when the mechanism for ejection and insertion is at the disk-in position without diskette.
    WRTPRT,         // !WRTPRT      This status bit is read as a zero only when a write protected diskette is in the drive or no diskette is inserted in the drive.
    TKO,            // !TKO         This status bit is read as a zero when a head is on track 00 or outer position of track 00. NOTE: rrKO is an output signal of a latch whose status is decided by the track 00 sensor only while the drive is not in power save mode.
    TACH,           //              tachometer - generated by the RP2040 clock device
    DATA1,          //              read data - not from latch but comes from RMT device
    na1101,         //              not assigned in latch
    READY,          // !READY       This status line is used to indicate that the host system can read the recorded data on the disk or write data to the disk. IREADY is a zero when the head position is settled on disired track, motor is at the desired speed, and a diskette is in the drive.
