...
void fujimail_read_hotspot(uint16_t offset)
{
    ...
    case (FN_H_REGSEL >> 8):
         ...
         * bus-serving layer, inline -- FN_HOT_SWAP (which must take effect
         * before the next read). They are no-ops here, and they
         * deliberately do NOT disturb an armed regsel, so a stray in this
         * page cannot break up a legitimate pair in the other. */
        break;
