 ...
 *            test_fujimail.c ../src/fujimail.c ../src/arcmap.c
 */
 ...
 * device both mirror. */
static uint8_t store[ARCMAP_WINDOW];
static struct {
...
static uint8_t stub_stream_open(int stream, uint32_t size)
{
    ...
    return (stream == 0) ? arcmap_gate(size) : 0;
}
...
static void test_unpaired_regdata(void)
{
    ...
    /* The swap hotspot (and any other 0x80+ special op) belongs to the
     * bus-serving layer; reaching this decoder as a stray it is a no-op and
     * leaves an armed regsel alone, same as every special op. */
    fujimail_read_hotspot(FN_H_REGSEL + FN_REG_DEVICE);
    fujimail_read_hotspot(FN_H_REGSEL + FN_HOT_SWAP);
...
static void test_dbc_abort_and_toobig(void)
{
    ...
    /* An image too big for the 8K window is refused at OPEN, before the
    ...
    /* A scripted OPEN refusal from the port is surfaced the same way. */
    fresh();
    pushed.open_err = FN_BOOT_ERR_TRUNCATED;
    dbc_open(0, 4096);
    assert(window[FN_R_BOOT_STATE] == FN_BOOT_FAILED);
    assert(window[FN_R_BOOT_ERR] == FN_BOOT_ERR_TRUNCATED);
    reg_write(FN_REG_BOOTLOCK, FN_BOOTLOCK_MAGIC);
...
/* A commit-time failure must never read as READY, and BOOTLOCK must stay
 * refused. */
static void test_dbc_close_fail(void)
{
    static uint8_t image[ARCMAP_WINDOW];

    fresh();
    pushed.close_err = FN_BOOT_ERR_TRUNCATED;
