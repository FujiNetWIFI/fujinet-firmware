...
// PiRTOII-Fuji: RP2040 (Intellivision cart bus) + ESP32-S3-WROOM-1-N16R8 on
// one board, no external RP2040 USB port. Reuses the Freenove ESP32-S3 CAM
// pin numbers from fujiversal-rs232.h/fujiversal-drivewire.h verbatim for
// SD/UART/LED so those paths need no firmware change, and adds the two
// RP2040 control signals that variant never needed. See
...
// board's single USB-C connector -- not the RP2040 link (that's native USB,
// permanently wired to the S3's USB host pins, no GPIO of its own).
#define PIN_UART0_RX             GPIO_NUM_44
#define PIN_UART0_TX             GPIO_NUM_43

// RP2040 hardware BOOTSEL forcing (recovery path, works even if the RP2040
// firmware is bricked or hung -- see fuji_mailbox.h's FUJI_MB_BOOTSEL_DOORBELL
// for the normal, cooperative path over the internal USB/PICOBOOT link
// instead). Each drives one transistor: active low, Hi-Z/input when idle so
// normal RP2040 boot/flash access is never disturbed.
//   PIN_RP2040_RUN:      -> RP2040 RUN, pulsed low then released to reset it
//   PIN_RP2040_BOOTSEL:  -> RP2040 QSPI_SS via 1k, held low through the pulse
//                           to mimic the BOOTSEL button
