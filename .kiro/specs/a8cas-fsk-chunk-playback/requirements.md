...
This feature adds standards-based reproduction of A8CAS `"fsk "` chunks within the normal FUJI cassette playback path, following the authoritative A8CAS CAS format specification (https://a8cas.sourceforge.net/format-cas.html). The `"fsk "` type is treated as one chunk type among others. The feature supports both CAS images that interleave `FUJI`, `baud`, `data`, and `fsk ` chunks and pure raw-FSK CAS images that contain a `FUJI` header followed only by one or more `fsk ` chunks with no `baud` or `data` chunks present. The change must preserve compatibility with all existing cassette formats and must not introduce any filename-, game-, or country-specific detection. Detection of `fsk ` chunks at runtime is performed solely by the 4-byte chunk type.

Real-world acceptance is validated against a multi-fixture corpus of locally held CAS images rather than a single file. These images are used only as local/manual acceptance fixtures; they are not committed to the FujiNet repository, are not used as automated CI fixtures, are not redistributed, and their filenames are never used as playback inputs. Their purpose is to validate that generic A8CAS `fsk ` playback reproduces real-world signal patterns correctly. For validation and source-correlation purposes only, expected signal structures may be cross-referenced against the original Turbo Software source published at https://github.com/a8dogdark/atari_turbo_software. Any binary or signal signatures derived from those sources are test evidence only and are never used as runtime detection inputs.

These requirements describe behavior only. The hardware mechanism used to reproduce raw FSK signals, the concrete PC-build behavior for FSK chunks, the memory or streaming strategy used for FSK payloads (including large chunks), and any acceptable hardware timing tolerances are out of scope for this document and are to be determined and justified in the Technical Design and the Test Plan.

## Glossary

- **CAS_Image**: An A8CAS-format Atari cassette image file, beginning with a `FUJI` header chunk and containing a sequence of chunks. A valid CAS_Image may consist of a `FUJI` header followed by one or more `fsk ` chunks with no `baud` or `data` chunks present (a pure raw-FSK image), or may interleave `FUJI`, `baud`, `data`, and `fsk ` chunks.
...
- **EOT (End Of Tape)**: The end of the CAS_Image, reached when all Chunks have been processed in ascending file order.
- **Real_World_Acceptance_Corpus**: A set of local CAS images used for manual and analytical acceptance testing, comprising at minimum `tt_international.cas`, `tt_river_raid.cas`, and `turbo_software_missile_command.cas`. Redistribution rights for these files are unknown; the corpus is not committed to the FujiNet repository, is not used as automated CI fixtures, is not redistributed, and its filenames are never used as playback or detection inputs. The image `Night Knight.cas` is NOT a member of the Real_World_Acceptance_Corpus and SHALL NOT be used as evidence of Turbo Software compatibility unless it is independently demonstrated to correspond to the referenced Turbo Software implementation.
- **Source_Correlated_Signature**: Any binary, signal, or decoded-data structure derived from the original Turbo Software source reference at https://github.com/a8dogdark/atari_turbo_software (including `STANDART_600_BAUDIOS/turbo600.asm`, `STAC_800_BAUDIOS/turbo800.asm`, and `FUJINET/TURBOFUJ.ASM`) and used for validation and source correlation. A Source_Correlated_Signature is test evidence only and is never used as a runtime detection input.
...
5. THE Cassette_Subsystem SHALL treat `fsk ` as a Chunk_Type that may appear either as the only signal-bearing Chunk_Type in a CAS_Image or interleaved with `FUJI`, `baud`, and `data` Chunk_Types.
...
8. WHEN a CAS_Image contains a `FUJI` header followed only by one or more well-formed `fsk ` Chunks, THE Cassette_Subsystem SHALL reproduce every FSK_Chunk in ascending file order to EOT without requiring the presence of any `baud` or `data` Chunk.
...
6. WHEN a malformed, truncated, or odd-length FSK_Chunk is encountered, THE Cassette_Subsystem SHALL deterministically terminate processing of that FSK_Chunk and continue normal playback control flow without hanging or crashing, AND WHERE no complete subsequent Chunk exists after the malformed or truncated FSK record, THE Cassette_Subsystem SHALL terminate normal playback control flow at EOT.
...
### Requirement 9: Real-world raw-FSK and interleaved playback (multi-fixture acceptance)

**User Story:** As a FujiNet user with real-world A8CAS cassette images, I want both interleaved (`baud`/`data`/`fsk `) images and pure raw-FSK images to play back correctly, so that generic A8CAS `fsk ` support is demonstrated against a corpus of real images rather than a single title-specific fixture.

#### Acceptance Criteria

1. WHEN a CAS_Image interleaves `baud`, `data`, and `fsk ` Chunks, THE Cassette_Subsystem SHALL process every Chunk in ascending file order to EOT, reproducing each `fsk ` Chunk as an FSK_Chunk and transmitting each `data` Chunk at the Active_Baud_Rate, until all Chunks in the CAS_Image have been processed.
2. WHEN a CAS_Image consists of a `FUJI` header followed by one or more `fsk ` Chunks with no `baud` or `data` Chunks present, THE Cassette_Subsystem SHALL reproduce every FSK_Chunk in ascending file order to EOT, honoring the Inter_Record_Gap and signal of each FSK_Chunk, without requiring any `baud` or `data` Chunk to be present.
3. WHEN the Cassette_Subsystem encounters a `baud` Chunk, THE Cassette_Subsystem SHALL set the Active_Baud_Rate to the value specified by that `baud` Chunk and apply it to every subsequent `data` Chunk until the next `baud` Chunk is encountered.
4. IF the Cassette_Subsystem encounters a Chunk whose Chunk_Type is not `baud`, `data`, or `fsk `, THEN THE Cassette_Subsystem SHALL skip that Chunk, continue processing the next Chunk in file order, and retain the current Active_Baud_Rate and playback position.
5. THE Cassette_Subsystem SHALL reproduce every CAS_Image solely from the contents of its A8CAS Chunks and SHALL NOT apply any filename-specific, game-specific, or country-specific logic.
6. WHERE real-world acceptance is performed, THE acceptance SHALL be validated against more than one member of the Real_World_Acceptance_Corpus, so that successful playback is not established from a single fixture.
7. THE Real_World_Acceptance_Corpus SHALL comprise at minimum the local fixtures `tt_international.cas`, `tt_river_raid.cas`, and `turbo_software_missile_command.cas`, which SHALL remain local/manual acceptance fixtures only and SHALL NOT be committed to the FujiNet repository, used as automated CI fixtures, or redistributed.
8. WHERE a Source_Correlated_Signature derived from the Turbo Software source reference is used, THE signature SHALL be used only as test evidence for validation and source correlation and SHALL NEVER become a runtime filename, game, country, magic-value, or signature detection input; runtime detection of `fsk ` Chunks SHALL remain purely by the 4-byte Chunk_Type.
9. WHERE hardware acceptance is performed, THE acceptance SHALL require successful playback on a physical Atari through physical FujiNet hardware for multiple members of the Real_World_Acceptance_Corpus.
10. IF a member of the Real_World_Acceptance_Corpus that conforms to the A8CAS format fails hardware playback, THEN the feature SHALL NOT be considered fully validated until the failure is corrected. A corpus member MAY be excluded from the validation set ONLY WHERE analysis demonstrates that its failure is caused by malformed, non-A8CAS, or otherwise explicitly out-of-scope behavior; a conforming A8CAS fixture SHALL NOT be excluded merely by documenting its failure.
11. THE image `Night Knight.cas` SHALL NOT be treated as a member of the Real_World_Acceptance_Corpus, and THE Cassette_Subsystem's Turbo Software compatibility SHALL NOT be evidenced by `Night Knight.cas` UNLESS that image is independently demonstrated to correspond to the referenced Turbo Software implementation.

### Requirement 10: Support large valid FSK chunks across the full A8CAS length range

**User Story:** As a FujiNet user with real-world raw-FSK cassette images, I want valid large FSK chunks to reproduce correctly, so that implementation resource constraints do not silently reject A8CAS-valid images.

#### Acceptance Criteria

1. WHEN the FUJI_Playback_Path processes an FSK_Chunk whose declared `chunk_length` is any value in the range 0 to 65535 inclusive (the maximum expressible by the A8CAS unsigned 16-bit chunk-length model) and whose payload lies entirely within the CAS_Image, THE Cassette_Subsystem SHALL correctly reproduce every complete FSK_Signal_Value present in the payload, subject only to actual platform resource availability, WHERE the handling of an odd `chunk_length` (including the trailing unpaired byte) remains governed by Requirement 6.4 and is not, by itself, treated as a well-formed reproducible-in-full payload.
2. THE Cassette_Subsystem SHALL NOT reject an otherwise-valid FSK_Chunk solely because its `chunk_length` is large within the A8CAS-permitted range, and SHALL NOT impose an FSK payload-size limit smaller than the A8CAS format permits for implementation convenience.
3. IF platform resource limitations prevent reproduction of an otherwise-valid FSK_Chunk, THEN THE Cassette_Subsystem SHALL fail safely without hanging, crashing, reading past the file boundary, or corrupting subsequent cassette state.
4. THE memory or streaming strategy used to reproduce large FSK_Chunks (whether the payload is pre-read, streamed, buffered incrementally, or otherwise emitted) is a Technical Design decision and SHALL NOT be constrained by these requirements; these requirements state only the observable behavior of correct reproduction within the A8CAS length bounds.
