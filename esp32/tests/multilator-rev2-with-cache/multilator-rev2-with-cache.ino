/**
   Test #40 - Multilator Rev2 Rewrite with dircache
*/

#define TEST_NAME "#FujiNet Multi-Diskulator with dircache"
...
/**
   Cache newly opened directory
*/
bool cache_dir(unsigned char hostSlot)
{
...
#ifdef DEBUG_VERBOSE
  Debug_printf("Cache opened for host slot %u\n", hostSlot);
#endif

  while (tnfs_readdir(hostSlot))
  {
    yield();
    dirCacheIdxEntry.off = datDirCache[hostSlot].position(); // why the hell isn't this tell() ?!
    dirCacheIdxEntry.len = strlen(current_entry);
    dirCacheIdxEntry.is_dir = 0; // Replace with tnfs_is_dir call.
    datDirCache[hostSlot].write((byte *)&current_entry, strlen(current_entry));
    idxDirCache[hostSlot].write((byte *)&dirCacheIdxEntry, sizeof(dirCacheIdxEntry));
#ifdef DEBUG_VERBOSE
    Debug_printf("Add Entry: %s\n", current_entry);
    Debug_printf("Add Offset: %u\n", dirCacheIdxEntry.off);
    Debug_printf("Add Len: %u\n", dirCacheIdxEntry.len);
#endif
  }
...
/**
   Erase cache
*/
void clear_cache(unsigned char hostSlot)
{
  char datFile[14] = "/datDirCacheX";
  char idxFile[14] = "/idxDirCacheX";
  ...
  datDirCache[hostSlot].close();
  idxDirCache[hostSlot].close();
  SPIFFS.remove(datFile);
  SPIFFS.remove(idxFile);
#ifdef DEBUG_VERBOSE
  Debug_printf("Cache closed and removed.\n");
#endif
}
...
bool cache_read(unsigned char hostSlot)
{
  ...
  Debug_printf("Get Offset: %u\n", dirCacheIdxEntry.off);
  Debug_printf("Get Len: %u\n", dirCacheIdxEntry.len);
#endif
