...
void sio_wifi_scan()
{
  ...
  if (scanIndex>totalSSIDs)
    SIO_UART.write('E'); // Error
  else
  {    
    SIO_UART.write('C'); // Completed command
    scanIndex++;
    // Write data frame
    SIO_UART.write(ssidInfo.rawData,34);
...
#ifndef DEBUG_S
    BUG_UART.printf("Network Status 0x02x\n\n",status[0]);
#endif
  }
...
void setup() 
{
  ...
  BUG_UART.println();
  BUG_UART.println("#AtariWifi Test Program #11 started");
#else
