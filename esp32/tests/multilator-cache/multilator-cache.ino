/**
   Test #40 - Multilator Rev2 with Dir Cache
*/

#define TEST_NAME "#FujiNet Multi-Diskulator /w Cache"
...
File dirCacheST[8];
bool is_dir;
unsigned short dir_cache_pos[8];
unsigned short dir_cache_num[8];
...
void sio_tnfs_open_directory()
{
  ...
  {
    if (tnfs_cache_directory(hostSlot))
      sio_complete();
    else
      sio_error();
  }
...
*/
void sio_tnfs_seek_directory()
{
  byte hostSlot = cmdFrame.aux1;
  unsigned short tmp = 0;
  byte ck = sio_to_peripheral((byte *)&tmp, sizeof(tmp));

  if (sio_checksum((byte *)&tmp, sizeof(tmp)))
  {
    dir_cache_pos[hostSlot] = tmp;
    sio_complete();
  }
  else
  {
...
/**
   sio_tnfs_tell_directory()
*/
void sio_tnfs_tell_directory()
{
  byte hostSlot = cmdFrame.aux1;
  byte err = false;
  ...
  sio_to_computer((byte *)&dir_cache_pos[hostSlot], 2, err);
}
...
void sio_tnfs_read_directory_entry()
{
  ...
  {
    // Read idx
    dirCacheIX[hostSlot].seek(((dir_cache_pos[hostSlot])*sizeof(dirCacheIdxEntry)), SeekSet);
    dirCacheIX[hostSlot].read((byte *)&dirCacheIdxEntry, sizeof(dirCacheIdxEntry));

    // Get FN
    dirCacheFN[hostSlot].seek(dirCacheIdxEntry.pos, SeekSet);
    dirCacheFN[hostSlot].read((byte *)&current_entry, dirCacheIdxEntry.len);

    // Get Attr
    dirCacheST[hostSlot].seek(dir_cache_pos[hostSlot]++, SeekSet);
    dirCacheST[hostSlot].read((byte *)&is_dir, 1);
...
void sio_tnfs_close_directory()
{
  ...
  {
    tnfs_cache_close(hostSlot);
    dir_cache_pos[hostSlot]=dir_cache_num[hostSlot]=0;
    sio_complete();
  }
  else
    sio_error();
...
/**
   Open 'autorun.atr'
*/
bool tnfs_stat(unsigned char hostSlot, char* filename)
{
  int start = millis();
  int dur = millis() - start;
  int c = 0;
...
/**
   TNFS Cache directory
*/
bool tnfs_cache_directory(unsigned char hostSlot)
{
  char fn[10] = "dirCacheX";
  char ix[10] = "idxCacheX";
  char st[10] = "staCacheX";

  fn[8] = hostSlot + 0x31;
  ix[8] = hostSlot + 0x31;
  st[8] = hostSlot + 0x31;

  dir_cache_num[hostSlot] = dir_cache_pos[hostSlot] = 0;

  dirCacheFN[hostSlot] = SPIFFS.open(fn, "w+");
  dirCacheIX[hostSlot] = SPIFFS.open(ix, "w+");
  dirCacheST[hostSlot] = SPIFFS.open(st, "w+");

  while (tnfs_readdir(hostSlot))
  {
    long pos;
...
/**
   Close directory cache
*/
void tnfs_cache_close(unsigned char hostSlot)
{
  char fn[10] = "dirCacheX";
  char ix[10] = "idxCacheX";
  char st[10] = "staCacheX";
  ...
  dirCacheFN[hostSlot].close();
  dirCacheIX[hostSlot].close();
  dirCacheST[hostSlot].close();
