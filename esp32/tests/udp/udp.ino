/**
   Test #36 - Multilator Rev2 Rewrite w/ UDP
*/

#define TEST_NAME "#FujiNet Multi-Diskulator /w UDP"
...
void sio_udp_begin()
{
  byte portL = cmdFrame.aux1;
  byte portH = cmdFrame.aux2;
  int port = (256 * portH) + portL;
  UDP.begin(port);
  sio_complete();
#ifdef DEBUG
  Debug_printf("Listening for UDP connections on port %d", port);
#endif
}
...
void sio_udp_status()
{
  ...
  {
    status[0] = clientUDP.remoteIP()[0];
    status[1] = clientUDP.remoteIP()[1];
    status[2] = clientUDP.remoteIP()[2];
    status[3] = clientUDP.remoteIP()[3];
  }
  else if (cmdFrame.aux1 == 0x02) // remote port #
  {
    p = clientUDP.remotePort();
    status[0] = p & 0xFF;
    status[1] = p >> 8;
  }
  else                       // # of bytes waiting (call first)
  {
    l = clientUDP.parsePacket();
    strcpy(clientHost, convert_remote_ip_to_string().c_str());
    clientPort = clientUDP.remotePort();
    status[0] = l & 0xFF;
    status[1] = l >> 8;
  }
  sio_to_computer(status, 4, false);
}
...
void sio_udp_read()
{
  ...
  clientUDP.read(udpPacket, packetSize);

  sio_to_computer(udpPacket, packetSize, false);
}
...
void sio_udp_write()
{
  ...
  sio_to_peripheral((byte *)&udpPacket, packetSize);
  
  clientUDP.beginPacket(clientHost, clientPort);
  clientUDP.write(udpPacket, packetSize);
  clientUDP.endPacket();
...
{
  sio_to_peripheral((byte *)&clientHost,sizeof(clientHost));
  sio_complete();
}
...
void setup()
{
  ...
  */
  ...
  cmdPtr[0xD4] = sio_udp_connect;
  cmdPtr[0xD3] = sio_udp_write;
  cmdPtr[0xD2] = sio_udp_read;
  cmdPtr[0xD1] = sio_udp_status;
  cmdPtr[0xD0] = sio_udp_begin;
