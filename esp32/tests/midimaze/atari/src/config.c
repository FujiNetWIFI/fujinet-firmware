...
/**
 * Enable midimaze mode
 */
void config_enable_midimaze(void)
{
  OS.dcb.ddevic=0x70;
  OS.dcb.dcomnd=0xF0; // enable midimaze
  OS.dcb.dstats=0x00; // No r or w
  OS.dcb.dbuf=NULL;   // not needed
  OS.dcb.dtimlo=0x0F; // Timeout
  OS.dcb.dbyt=0;      // not needed
  OS.dcb.daux=0;      // not used
  siov();
}
...
void config_wait(void)
{
  ...
  screen_puts(0, 0,"#FUJINET READY FOR");
  screen_puts(0,20,"     MIDIMAZE     ");

  screen_puts(0,4,"If you are using a MIDI MAZE Cart");
  screen_puts(0,5,"Your #FUJINET should be on ext pwr!");

  screen_puts(0,7,"Press 'M' to enable midi-maze mode,");
  screen_puts(0,8,"Turn off pwr and insert cartridge.");
  screen_puts(0,9,"Or any other key to boot from Fujinet.");
  
  screen_puts(0,12,"Fuji version of MIDIMAZE Req's 256K!");

  k=cgetc();

  if (k=='m')
    {
      config_enable_midimaze();
      screen_puts( 0,21,"MIDIMAZE ENABLED");
      screen_puts(20,21,"TURN OFF POWER..");
      for (;;) { }
    }
 ...
 * Get remote host
 */
void config_get_remote_host(void)
{
  screen_clear();
  bar_clear();
  ...
  while(successful==false)
    {
      ...
      screen_puts(0,1,"If you are using a MIDI MAZE Cart");
      screen_puts(0,2,"Your #FUJINET should be on ext pwr!");
