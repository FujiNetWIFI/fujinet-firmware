...
// In-memory source, so the tests touch no filesystem and no fnFile (which is
// FileHandler on some targets and std::FILE on others).
class MemSource : public ImdSource
{
public:
    MemSource(std::vector<uint8_t> d, bool w) : data(std::move(d)), _w(w) {}

    bool read_at(uint32_t off, void *dst, uint32_t len) override
    {
        if (off > data.size() || len > data.size() - off)
            return false;
        memcpy(dst, data.data() + off, len);
        return true;
    }

    bool write_at(uint32_t off, const void *src, uint32_t len) override
    {
        if (!_w || off > data.size() || len > data.size() - off)
            return false;
        memcpy(data.data() + off, src, len);
        return true;
    }

    uint32_t size() override { return (uint32_t)data.size(); }
    bool     writable() const override { return _w; }
...
// Keeps source and image together; ImdImage does not own the source.
struct Mounted
{
    MemSource src;
    ImdImage  img;
    ...
    Mounted(const Imd &b, bool writable = false) : src(b.out, writable)
    {
        st = img.open(&src, writable);
    }
};
...
TEST_CASE("in-place writes")
{
    ...
    SUBCASE("a compressed record takes a uniform buffer")
    {
        ...
        CHECK(m.src.data.size() == b.out.size());
    }
