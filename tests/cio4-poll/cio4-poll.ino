/**
 * Test #18 - CIO Test #4 - SIO Poll of CIO3
 */

#include <ESP8266WiFi.h>
#include <FS.h>
#include "atari/handler.h"
...
unsigned char loader_bin[] = {
  ...
  0xa9, 0x0f, 0x8d, 0x06, 0x03, 0xa9, 0x00, 0x8d, 0x07, 0x03, 0xa9, 0x94,
  0x8d, 0x08, 0x03, 0xa9, 0x05, 0x8d, 0x09, 0x03, 0xa9, 0x00, 0x8d, 0x0a,
  0x03, 0x8d, 0x0b, 0x03, 0x20, 0x59, 0xe4, 0x4c, 0x01, 0x1d
};
unsigned int loader_bin_len = 70;
...
void sio_get_id()
{
  ...
  if (cmdFrame.devic == 0x31 || cmdFrame.devic==0x50 || cmdFrame.devic==0x70)
    cmdState = COMMAND;
  else
  {
    cmdState = WAIT;
 ...
 * Send bootstrap
 */
void sio_send_bootstrap()
{
  byte ck=sio_checksum((byte *)&loader_bin, loader_bin_len);
 ...
 * Send N: handler
 */
void sio_send_n_handler()
{
  byte ck=sio_checksum((byte *)&autorun_hdl, autorun_hdl_len);
...
/**
 * SIO type 1 poll, return a buffer containing a DCB
 */
void sio_type1_poll()
{
  byte dcb[0x0C]=
  {
    0x70, // DDEVIC
    0x01, // DUNIT
    0x21, // DCOMND = '!' Load bootstrap
    0x40, // DSTATS = READ from peripheral
    0x00, // DBUF = $0500
    0x05, // "      "
    0x0F, // DTIMLO
    0x00, // UNUSED
    loader_bin_len, // DBYTL
    0x00, // DBYTH
    0x00, // DAUX1
    0x00, // DAUX2
  };
...
#ifdef DEBUG
  Debug_printf("Responding to type 1 poll.\n");
#endif
