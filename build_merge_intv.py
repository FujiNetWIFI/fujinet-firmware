...
# partitions.bin, firmware.bin -- the last one already carrying the RP2040
# firmware, see build_pico_intv.py), merges them into ONE file via
# `esptool.py merge_bin` so `esptool.py write_flash 0x0 <merged file>` is
# the only command needed to get a fully working ESP32-S3 + embedded RP2040
# image onto a chip.
#
...
# config data) -- that's unrelated to the RP2040 hand-off this exists for,
# and stays on the existing, orthogonal `-t uploadfs` path for first-time
# setup (see build.sh -f).
#
...
# note flasher_args.json currently says flash_size "4MB" despite this being
# a 16MB chip (a pre-existing inconsistency in this board's sdkconfig, not
# something introduced here); using it as-is keeps the merged image
# byte-for-byte consistent with the normal upload path rather than
# introducing a second, different answer.
#
