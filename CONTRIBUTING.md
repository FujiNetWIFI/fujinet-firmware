...
C++ and code-structure rules live in [`docs/cpp-style.md`](docs/cpp-style.md).

## Before you start

No document can tell you what is currently planned. Work is often staged as a series of topic
branches (`git branch -r` shows things like `heap-02-leaks` through `heap-05-internal-dram`, plus
several `*-bringup` branches), so the area you are about to change may already be in flight.

- Check `git log --oneline -20 -- <paths>` and `git branch -r` before touching an area. If recent
  work or an open branch overlaps, raise it before proceeding rather than duplicating or conflicting.
- For anything beyond a contained fix — a new abstraction, a new device or bus, a change to a shared
  ...
  before writing the code. A rejected design costs a reviewer far more than a question does.
- If a maintainer asks you to hold off because another change is coming, stop. Do not rework the
  change to land it sooner.
- Ask on Discord or in the issue if any of this is unclear. That is cheaper than a rewrite.
