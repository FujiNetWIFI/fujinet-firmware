...
if [ ${UPLOAD_IMAGE} -eq 1 ] ; then
  if [ "${BUILD_BOARD}" = "fujiversal-intv" ] ; then
    # fujiversal-intv carries a second firmware (the RP2040's, embedded at
    # build time -- see build_pico_intv.py / build_merge_intv.py) inside its
    # own app image, and build_merge_intv.py's post-action already folded
    # bootloader+partition-table+app into one file after the build above.
    # Flash that single merged file directly with esptool.py instead of
    # `pio run -t upload`'s normal three-piece upload, so "one file, one
    ...
    echo "=============================================================="
    echo "Flashing merged fujiversal-intv image (ESP32-S3 + embedded RP2040 firmware)"
    ...
    echo ""
    echo "ESP32-S3 flashed. The RP2040 firmware embedded in that image will be pushed"
    echo "to the RP2040 automatically the next time it attaches to the S3 in BOOTSEL"
    echo "mode (power-cycle it holding BOOTSEL) -- see PicobootClient.cpp."
  else
