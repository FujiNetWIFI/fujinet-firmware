...
union
{
  struct
  {
    unsigned long magicNumber=0xDEADBEEF; // 4
    char ssid[32]; // 32
    char key[64]; // 64
    byte reserved[412]; // Reserved block.
  };
  byte rawData[512];
} ee;
...
void sio_set_new_net_info()
{
  ...
  {
#ifdef DEBUG_S
    Serial1.printf("Write net info to eeprom success.\n");
#endif
    delayMicroseconds(DELAY_T5);
    Serial.write('C');
    EEPROM.put(0,ee.rawData);
    EEPROM.commit();
    networkUpdate=true;
  }
  else
  {
#ifndef DEBUG_S
    Serial1.printf("Error while receiving net info from computer.\n");
#endif
    delayMicroseconds(DELAY_T5);
    Serial.write('E');
  }
