...
void sio_write()
{
  ...
  if (ck == sio_checksum(sector, 128))
  {
    ...
    {
      inter.seek(offset, SeekSet);
      inter.write(sector, 128);
      inter.flush();
    }
    Serial.write('C');
    yield();
...
void loop()
{
  ...
  if ( !wificlient.connected() && WiFi.status() == WL_CONNECTED )
  {
    ...
    wificlient.println("#FujiNet Interrupt Test");
  }
#endif
...
#ifdef DEBUG
    Debug_printf("sioclient INTLOW\n");
#endif
    digitalWrite(PIN_INT, LOW); // Active LOW
    delayMicroseconds(700);
    digitalWrite(PIN_INT, HIGH); 
#ifdef DEBUG
    Debug_printf("sioclient INTHIGH\n");
#endif
  }
  ...
  if ((sioserver != NULL) && (sioserver->available()))
  {
    // Wiggle INTERRUPT line
#ifdef DEBUG
    Debug_printf("sioserver INTLOW\n");
#endif
    digitalWrite(PIN_INT, LOW); // Active LOW
    delayMicroseconds(700);
    digitalWrite(PIN_INT, HIGH);
#ifdef DEBUG
    Debug_printf("sioserver INTHIGH\n");
#endif
